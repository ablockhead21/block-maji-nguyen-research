...
\onslide<4>{
\draw [->, > = stealth, thick, dashed, color = red] (alice) --(bob) node[midway, above]{$ \mathcal{L}_{Bob}(r_A) $};
}

\onslide<5>{
	\draw [->, > = stealth, thick, dashed, color = red] (bob) --(alice) node[midway, above]{$ \mathcal{L}_{Alice}(r_B) $};
}
