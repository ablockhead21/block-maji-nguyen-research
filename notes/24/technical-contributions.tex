...
\begin{frame}{CorrExt Construction Overview II}
	...
	\only<5>{Natural generalization of the \cite{C:GIMS15} protocol for \IP[] over \GF{2} to \IP[] large fields.}
	\only<6-8>{Embed $ m $ copies of $\OLE[\GF{2}] $ into \underline{one} $ \OLE[\bbK] $}
	\only<7>{\begin{center}
			\textcolor{red}{\huge One of Our Technical Contributions}
	\end{center}}
