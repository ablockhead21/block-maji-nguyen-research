...
\begin{frame}{CorrExt Construction Overview II}
	...
	\only<2>{Natural generalization of the \cite{C:GIMS15} protocol for \IP[] over \GF{2} to \IP[] large fields.}
	\only<3-5>{Embed $ m $ copies of $\OLE[\GF{2}] $ into \underline{one} $ \OLE[\bbK] $}
	\only<4>{\begin{center}
			\textcolor{red}{\huge Our Contribution}
	\end{center}}
	\only<5>{\setbeamercolor{block title}{bg=Gray, fg=white}
		\begin{block}{Notes}
			\begin{itemize}
...
\begin{frame}{Our Combinatorial Problem}
	...
	\begin{block}{Our Combinatorial Problem}
		...
		\begin{itemize}
			\item $ s_i$ and $ t_i $ are non-negative integers
			\item $ s_i + t_i < n $
	...
	\begin{block}{Notes}
		\begin{itemize}
			\onslide<2->{\item This problem is a generalization of the 3-free set problem\only<2>{\footnote{3-free set problem is a problem of constructing dense sets of integers that are free of arithmetic progressions of length 3}} %(sets that avoid any arithmetic progressions of length 3) problem.
			\begin{itemize}
			...
			\item In an ongoing work, for our combinatorial problem, we show that $ m = o(n) $}
		\end{itemize}
