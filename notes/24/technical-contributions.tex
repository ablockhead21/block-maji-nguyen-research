...
\begin{frame}{CorrExt Construction Overview II}
	...
	\only<6-8>{Embed $ m $ copies of $\OLE[\GF{2}] $ into \underline{one} $ \OLE[\bbK] $, where $ \bbK $ is a large field of characteristic 2}
	\only<7>{\begin{center}
...
\begin{itemize}
	\item Applies only to ``multiple independent samples of small correlations"
